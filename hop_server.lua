--[[
    ...
    Faz re-scan imediato antes de qualquer teleporte.
    Usa queue_on_teleport para auto-executar no novo servidor.
...
local function fetchServers(onDone, onProgress)
    ...
    task.spawn(function()
        ...
        repeat
            ...
            if onProgress then
                onProgress(
                    string.format("Pagina %d/%d...", page, Config.MaxPages),
...
local function queueAutoReload()
    ...
    pcall(function()
        local src = string.format(
            'task.wait(3) pcall(function() loadstring(game:HttpGet(%q,true))() end)',
            Config.SelfURL
        )
        local fn = nil
        ...
        if type(fn) == "function" then
            ...
            print("[HopServer] queue_on_teleport indisponivel -- reexecute manualmente.")
        end
    end)
end
...
for _, btn in ipairs({MinBtn, CloseBtn}) do
    ...
    btn.MouseEnter:Connect(function()
        tw(btn, { BackgroundColor3 = orig:Lerp(Color3.new(1,1,1), 0.15) }, 0.1)
    end)
    btn.MouseLeave:Connect(function()
        tw(btn, { BackgroundColor3 = orig }, 0.1)
    end)
end
...
local function makeButton(text, bgColor, w, order, parent)
    ...
    btn.MouseEnter:Connect(function()
        tw(btn, { BackgroundColor3 = bgColor:Lerp(Color3.new(1,1,1), 0.12) }, 0.1)
    end)
    btn.MouseLeave:Connect(function()
        tw(btn, { BackgroundColor3 = bgColor }, 0.1)
    end)
    return btn
end
...
local function pColor(playing, max)
    ...
    if playing / math.max(max, 1) < 0.5 then return C.Yellow end
    return C.Red
end
...
local function rankCol(i)
    if i == 1 then return C.Gold   end
    if i == 2 then return C.Silver end
    if i == 3 then return C.Bronze end
    return C.Border
end
...
local function renderList(servers)
    ...
    for i, sv in ipairs(servers) do
        ...
        make("TextLabel", {
            ...
            BackgroundTransparency = 1,
            TextSize = i <= 3 and 13 or 10,
            Font = Enum.Font.GothamBold,
            TextColor3 = i <= 3 and rankCol(i) or C.TextDim,
            TextXAlignment = Enum.TextXAlignment.Center,
        }, Card)

        -- Players
        make("TextLabel", {
            Text = sv.playing .. "/" .. sv.maxPlayers,
        ...
        if sv.playing == 0 then
            local badge = make("Frame", {
                ...
                BackgroundColor3 = C.Accent:Lerp(C.BG, 0.5),
                BorderSizePixel = 0,
            }, Card)
        ...
        local fpsBadge = make("Frame", {
            ...
            Position = UDim2.new(0, 122, 0.5, -10),
            BackgroundColor3 = fc:Lerp(C.BG, 0.7),
            BorderSizePixel = 0,
        }, Card)
        ...
        GoBtn.MouseEnter:Connect(function()
            tw(GoBtn, { BackgroundColor3 = C.AccentHov }, 0.1)
        end)
        GoBtn.MouseLeave:Connect(function()
            tw(GoBtn, { BackgroundColor3 = C.Accent }, 0.1)
        end)
        Card.MouseEnter:Connect(function()
            tw(Card, { BackgroundColor3 = C.CardHov }, 0.12)
        end)
        ...
        function(servers, ok)
            if ok then
                ...
                task.delay(0.6, function()
                    if ProgBar.Parent then
                        tw(ProgBar, { Size = UDim2.new(0, 0, 1, 0) }, 0.4)
                    end
                end)
...
-- Botao Ir ao Menor (re-scan antes de pular)
    ...
    doScan({
        onDone = function(servers, ok)
            ...
            if not ok or #servers == 0 then
                setStatus("Nenhum servidor disponivel", C.Red)
                return
            end
            local best = servers[1]
            setStatus(string.format(
                "Pulando... [%d/%d players | %d fps]",
